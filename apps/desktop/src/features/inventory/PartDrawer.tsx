...
export function PartDrawer({ open, editing, form, boxes, quantityText, error, onChange, onQuantityChange, onLookupLcsc, onSave, onRequestClose, onRequestCloseReady }: {
  ...
  return <Drawer open={open} title={editing ? "编辑器件" : "新增器件"} dirty={isPartDirty(form, quantityText, editing)} confirmDiscard={() => window.confirm("放弃未保存更改？")} onRequestClose={onRequestClose}>
    <form className="pn-drawer-form" onSubmit={(event) => { event.preventDefault(); onSave(); }}>
      ...
      <FormField label="数量" htmlFor="part-quantity"><input id="part-quantity" className="pn-control" type="number" min="0" inputMode="numeric" autoComplete="off" readOnly={Boolean(editing && editing.quantity > 0)} value={quantityText} onFocus={(event) => event.currentTarget.select()} onChange={(event) => onQuantityChange(event.target.value)} /></FormField>
      {field("note", "备注")}
